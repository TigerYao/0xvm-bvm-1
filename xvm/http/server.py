...
from xvm._utils.logging import get_logger
...
class HTTPServer(Service):
    ...
    def __init__(
        ...
        self.logger = get_logger('xvm.http.HTTPServer')

        # aiohttp logs every HTTP request as INFO so we want to reduce the general log level for
        # this particular logger to WARNING except if the Xvm is configured to write DEBUG2 logs
